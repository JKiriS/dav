$(document).ready(function(){
	...
	function load(type){
		$.post("/console/rs/get"+type,function(res){
			if(res.data){
	...
	$(".content").on("mouseenter",".console-group-content .status",function(){
	...
	$(".content").on("click",".console-group-content .status button",function(){
		var type = $(this).parents(".console-group").attr("id");
		var target = $(this).parents("tr").attr("id");
		var cmd = $(this).html();
		$.post("/console/rs/set",{'type':tyoe,'target':target,'cmd':cmd},function(res){
			if(res.data){
				$(this).parent().html(res.data);
			}
			else{
				alert(res.error);
			}
